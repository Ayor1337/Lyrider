...
- 支持系统托盘图标和跟随应用主题的快捷菜单；可设置点击窗口关闭按钮时最小化到托盘，双击托盘图标可恢复窗口
...
- 宽度低于 820 DIP 时，Cider 连接字段改为上下排列；低于 720 DIP 时，设置项的说明与控件也改为上下排列。设置页内容水平居中并支持纵向滚动，滚动条位于页面右侧；标题与内容左缘对齐，返回按钮和标题固定在页面顶部。
