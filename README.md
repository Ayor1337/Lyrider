...
任务栏播放状态默认关闭，可在设置页启用。该功能仅在 Windows 11 的主显示器水平任务栏上运行：播放条作为透明子窗口挂载到系统任务栏，并优先显示在原生 Widgets 按钮旁。Cider 未连接或没有当前歌曲时播放条会自动隐藏；Explorer 重启后会自动重新挂载。由于任务栏结构不是公开扩展接口，后续 Windows 更新或第三方任务栏修改工具可能导致定位暂时失效，此时 Lyrider 会隐藏播放条并继续重试，不会覆盖未知的系统控件。
...
src/Lyrider.TaskbarWidget/    WPF 透明任务栏子窗口、Shell 定位与播放条交互
tests/Lyrider.Tests/          队列刷新与响应解析回归测试
```

当前版本使用 HTTP 轮询，不包含 Socket.IO 或系统托盘；任务栏播放条复用主窗口的轮询结果，不会额外请求 Cider API。
