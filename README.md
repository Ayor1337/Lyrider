...
- 显示时间轴歌词，优先使用 Cider，并在其歌词接口不可用时回退到 LRCLIB；自动突出并滚动到当前行
- 支持点击时间轴歌词跳转播放位置
- 提供播放/暂停、上一首、下一首、随机、循环和音量控制
- 可选的 Windows 11 任务栏播放条：在系统小组件旁显示封面、当前歌词和下一句，当前句过长时自动横向滚动，悬浮后切换为播放控制；无时间轴歌词时回退显示歌名和歌手
- 支持系统托盘图标和跟随应用主题的 Fluent 快捷菜单，包含操作图标、悬停/键盘焦点反馈与弹出动画；可设置点击窗口关闭按钮时最小化到托盘，双击托盘图标可恢复窗口
- 显示 Cider 连接、认证及响应错误状态
- 标题栏左上角提供三点悬浮菜单，可查看 Cider 连接状态并切换队列、歌词、设置或退出应用
- 提供 Windows 11 风格的 Mica 分层与 Fluent 设置卡片，可调整主题、歌词字号、背景强度、默认视图、音量控件、窗口置顶和关闭行为
...
任务栏播放状态默认关闭，可在设置页启用。该功能仅在 Windows 11 的主显示器水平任务栏上运行：播放条作为透明子窗口挂载到系统任务栏，并优先显示在原生 Widgets 按钮旁。时间轴歌词可用时显示当前句和下一句，当前句过长时会在 216 DIP 宽度内自动横向滚动；歌词尚未开始、不可用或不带时间轴时回退显示歌名和歌手。悬浮播放条会暂停歌词滚动并切换为播放控制。未悬浮时，只有播放条背景对应的 216 × 40 DIP 区域会触发控制栏。Cider 未连接或没有当前歌曲时播放条会自动隐藏；Explorer 重启后会自动重新挂载。由于任务栏结构不是公开扩展接口，后续 Windows 更新或第三方任务栏修改工具可能导致定位暂时失效，此时 Lyrider 会隐藏播放条并继续重试，不会覆盖未知的系统控件。
